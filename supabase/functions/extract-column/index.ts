  ...
  try {
    ...
    const buildSystemPrompt = (lc: string, cp?: string) => {
      if (isSynthesisColumn) {
        return lc === 'pt'
          ? `Você é um assistente de pesquisa científica preciso.\nSua tarefa é ler o texto de artigos científicos e gerar um resumo RICO e ÚTIL que ajude o pesquisador a decidir se o paper é relevante.\n\n[REGRAS]\n1. Baseie seu resumo APENAS no texto fornecido.\n2. Estruture cada resumo com 3-5 frases cobrindo: (a) OBJETIVO do estudo, (b) MÉTODO/DESENHO utilizado (tipo de estudo, amostra, intervenção), (c) PRINCIPAIS RESULTADOS com dados quantitativos quando disponíveis (p-valor, OR, IC, tamanho amostral), (d) CONCLUSÃO principal.\n3. Se o abstract for muito curto ou vago, extraia o máximo possível dos metadados (journal, ano, autores) e do título para inferir contexto, indicando "(inferido do título/contexto)".\n4. NUNCA retorne apenas "Sem texto disponível" se houver título e metadados — use-os para gerar ao menos um resumo básico inferido.\n5. Somente se não houver NENHUMA informação (título vazio, sem metadados), retorne "Sem informações suficientes".\n6. Use o campo citation_context para incluir a frase mais representativa dos resultados.\n7. Responda APENAS usando a função fornecida.\n\n${cp ? `INSTRUÇÃO ESPECÍFICA: ${cp}` : ''}`
          : `You are a precise scientific research assistant.\nYour task is to read paper text and generate a RICH, USEFUL summary that helps the researcher decide if the paper is relevant.\n\n[RULES]\n1. Base summary ONLY on provided text.\n2. Structure each summary with 3-5 sentences covering: (a) OBJECTIVE of the study, (b) METHOD/DESIGN used (study type, sample, intervention), (c) KEY RESULTS with quantitative data when available (p-value, OR, CI, sample size), (d) Main CONCLUSION.\n3. If the abstract is very short or vague, extract as much as possible from metadata (journal, year, authors) and the title to infer context, marking with "(inferred from title/context)".\n4. NEVER return just "No text available" if there is a title and metadata — use them to generate at least a basic inferred summary.\n5. Only if there is NO information at all (empty title, no metadata), return "Insufficient information".\n6. Use citation_context for the most representative sentence about results.\n7. Respond ONLY using the provided function.\n\n${cp ? `USER INSTRUCTION: ${cp}` : ''}`;
      }
      return lc === 'pt'
