...
import { stripHtml } from "@/lib/writing/richText";
...
export const WritingDocumentVersionsDialog = ({ documentId, currentContent, onRestore }: Props) => {
  ...
  const { data: versions = [], refetch } = useQuery({
    queryKey: ["writing-document-versions", documentId],
    enabled: open,
    queryFn: async () => {
  ...
  const saveSnapshot = async () => {
    ...
    refetch();
    qc.invalidateQueries({ queryKey: ["writing-document-versions", documentId] });
  };

  const removeVersion = async (id: string) => {
    if (!confirm(locale === "pt" ? "Excluir versão?" : "Delete version?")) return;
    const { error } = await supabase.from("writing_document_versions").delete().eq("id", id);
    if (error) return toast.error(error.message);
    refetch();
  ...
  // Content is stored as HTML (rich-text editor) — diff on the readable plain text, restore keeps the full HTML.
  const parts = selected ? diffWords(stripHtml(selected.content || ""), stripHtml(currentContent || "")) : [];
      ...
      <DialogContent className="max-w-5xl h-[80vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center justify-between">
            <span className="flex items-center gap-2"><History className="h-4 w-4" />{locale === "pt" ? "Histórico do documento" : "Document history"}</span>
            <Button size="sm" variant="outline" onClick={saveSnapshot} className="gap-1.5">
              <Save className="h-3.5 w-3.5" />{locale === "pt" ? "Salvar versão nomeada" : "Save named version"}
            </Button>
          </DialogTitle>
        </DialogHeader>
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] text-muted-foreground mt-1">{stripHtml(v.content || "").length} {locale === "pt" ? "caracteres" : "chars"}</p>
                  </div>
...
export default WritingDocumentVersionsDialog;
